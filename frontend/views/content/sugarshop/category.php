<div id="sugarDates" class="clearfix bgContent m-t-3">
    ...
    <div class="html_carousel2">
      <div id="foo2">
        ...
        <div class="slide">
          ...
          <div>
            <h2><a href="javascript:void(0);"><?php echo $deals[0]->name;?></a></h2>
  ...
  <div class="cate">
    ...
    <div class="topCate">
      ...
      <?php echo form_open('sugarshop/searchcategory', array('id'=>'searchcategory', 'name'=>'searchcategory'))?>
          <select name="dealcategory" id="dealcategory" onchange="submitsearchcategory();">
            <option class="movie" <?php if($categoryID=="alle"){echo 'selected="true"';}?> value="alle">Alle</option>
            <?php if($dealcategorys){ foreach($dealcategorys as $row){ ?>
            <option <?php if($categoryID==$row->id){echo 'selected="true"';}?> class="movie" value="<?php echo $row->id;?>"><?php echo $row->name;?></option>
            <?php }}?>
          </select>
      </form>
    ...
    ?>
    <?php echo form_open('cart/insert.html', array('id'=>'pro_'.$rows->id))?>
    <li <?php if($i%3==0){echo 'class="end"';}?> >
        <a id="wishlist_<?php echo $rows->id;?>" href="javascript:void(0);" <?php if($user){?> <?php if(!$check){?> onclick="addWishlist('<?php echo $user->id;?>','<?php echo $rows->id;?>');" <?php }}else{?> data-reveal-id="f-errorPage" <?php }?> class="wishlist-<?php if($check){echo "on";}else{echo "off";}?> tooltip" style="cursor: pointer;"><span><?php echo $rows->name;?></span></a>
    ...
    <?php }else{?>
    <div style="padding: 10px 0;">Der er ingen produkt, der matcher søgekriterierne.</div>
    <div style="">
